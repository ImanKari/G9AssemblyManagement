...
namespace G9AssemblyManagement.Core
{
    ...
    /// <remarks>
    ///     The .NET framework does not provide navigation from attributes back to their targets, principally for the reason
    ///     that
    ///     in typical usage scenarios for attributes, the attribute is discovered by a routine which already has a reference
    ///     to a
    ///     member type.
    ///     There are, however, bona-fide cases where an attribute needs to detect it's target - an example is a localizable
    ///     sub-class of the
    ///     DescriptionAttribute. In order for the DescriptionAttribute to return a localized string, it requires a resource
    ///     key and, ideally,
    ...
    ///     Note to callers:
    ///     Your Attribute-derived class must implement Equals and GetHashCode, otherwise a run-time exception will occur,
    ///     since this class
        ...
        where TAttribute : System.Attribute
    {
        ...
        /// </summary>
        static G9CAttributeTargetHelper()
        {
            ...
            skipAssemblies =
                new List<string>(typeof(TAttribute).Assembly.GetReferencedAssemblies().Select(c => c.FullName));
        ...
        {
            const BindingFlags memberInfoBinding = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static |
            ...
            if (!skipAssemblies.Contains(assembly.FullName))
            {
                ...
                Debug.WriteLine("Loading attribute targets for " + typeof(TAttribute).Name + " from assembly " +
                                assembly.FullName);
